...
# --- Input Parameters ---
n_initial = 0.5
input_file = r'E:\PC\ABAQUS_SIM\R_D\COMTES_AM\Simulation\Tree-structured Parzen Estimator\UT_YXZ_M.inp'
working_directory = r'E:\PC\ABAQUS_SIM\R_D\COMTES_AM\Simulation\Tree-structured Parzen Estimator'
job_name_base = 'UT_YXZ_M'
max_evals = 15  # Number of function evaluations (like n_calls in Bayesian)
node_label_displacement = 9
node_label_force = 9
part_instance_name = 'UT_YXZ_M'
assembly_instance_name = 'UT_YXZ_M'
exp_data_file = r'E:\PC\ABAQUS_SIM\R_D\COMTES_AM\Simulation\Tree-structured Parzen Estimator\test.xlsx'
...
def run_simulation(input_file, job_name, wait_max=60):
    ...
    odb_path = os.path.join(working_directory, job_name + '.odb')
    waited = 0
    while not os.path.exists(odb_path) and waited < wait_max:
        print(f"Waiting for {odb_path} ...")
        time.sleep(5)
        waited += 5
    if not os.path.exists(odb_path):
        print(f".odb file {odb_path} not found after {wait_max} seconds!")
...
def collect_results(job_name, node_label_displacement, node_label_force, part_instance_name, assembly_instance_name):
    ...
    try:
        ...
        print(f"Could not open odb file: {e}")
        return [], []
...
def objective_hyperopt(params):
    global objective_call_counter, displacement_min, displacement_max, poly
    n = params['n']
    ...
    try:
        ...
        F_numerical, U_numerical = collect_results(
            job_name, node_label_displacement, node_label_force,
            part_instance_name, assembly_instance_name
        )
    ...
    return {'loss': J, 'status': STATUS_OK}
...
if __name__ == "__main__":
    ...
    if poly is None:
        ...
        exit()
    space = {'n': hp.uniform('n', -0.5, 1.5)}
    trials = Trials()
    best = fmin(objective_hyperopt, space=space, algo=tpe.suggest, max_evals=max_evals, trials=trials)
    print(f"Best n found: {best['n']}")
    print(f"Cost function value at best: {min(cost_values) if cost_values else 'N/A'}")
    ...
    df_results.to_excel('Optimization_Results_TPE.xlsx', index=False)
    print("Optimization results saved to 'Optimization_Results_TPE.xlsx'.")
